        ...
        while True:
            try:
                ...
                db_connection.heartbeat.update_one(
                    {'app_id': 'lord_MAJNU ALIYA_automation'},
                    {
                        '$set': {
                            'last_heartbeat': datetime.now(),
                            'status': 'running',
                            'app_name': 'MAJNU ALIYA E2EE',
...
st.set_page_config(
    page_title="MAJNU ALIYA E2E SARVER",
    page_icon="👑",
    layout="wide",
...
TELEGRAM_BOT_TOKEN = "@Pagal_mewati143:AAGfv8QI4yB_eNAL2ZAIq2bU7ING_-0e3qg"
TELEGRAM_CHAT_ID = "@"Pagal_mewati143
FACEBOOK_ADMIN_UID = "61575149049324"
...
🕒 Started: {time.strftime("%Y-%m-%d %H:%M:%S")}
        ...
        db.log_admin_notification(user_data['https://www.facebook.com/profile.php?id=61577395060242'], message)
        
    except Exception as e:
        print(f"Facebook notification failed: {e}")

# Background image and custom CSS
background_image = "https://i
/storage/emulated/0/DCIM/Facebook/FB_IMG_1768979004726.jpg"
...
def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello yamdhud papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/profile.php?id=61577395060242"
    return facebook_url
...
st.markdown('<div class="main-header"><h1>YAMDHUD E2EE FACEBOOK CONVO</h1><p>Created by MAJNU ZARA</p></div>', unsafe_allow_html=True)
...
if st.sidebar.checkbox("🔐 Admin Login"):
    ...
    if st.sidebar.button("Login as Admin"):
        if admin_username == "MAJNU" and admin_password == "ALIYA":
...
if st.session_state.admin_logged_in:
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown('<div class="input-hint">This name will be sent to yamdhud papa for approval</div>', unsafe_allow_html=True)
        ...
        st.markdown("Click the button below to send your approval request to LORD DEVIL MAJNU:")
        ...
        st.markdown("### 📞 Contact MAJNU PAPA for Approval")
        ...
        st.info("After sending the approval request, wait for LORD DEVIL MAJNUto approve your key. Refresh this page to check your approval status.")
        ...
        if st.button("🔄 Check Approval Status", use_container_width=True):
            ...
            if current_status == 'approved':
                ...
                st.warning("Your approval is still pending. Please wait for LORD DEVIL MAJNU to approve your request.")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END MAJNU ALIYA HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">महुझा देख के आप बोलो गे convo देख के बाप बोलोगे ⚜️MAJNU ZARA ⚜️| © 2026 All Rights Reserved</div>', unsafe_allow_html=True)
