...
def send_approval_request_via_whatsapp(user_real_name, approval_key):
    message = f"Hello majnu papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    whatsapp_url = f"https://wa.me/917818956614?text={requests.utils.quote(message)}"
    return whatsapp_url

def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello Majnu papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/share/1FXvZ1hvcJ/"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello majnu  papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    telegram_url = f"https://@Pagal_mewati143?text={requests.utils.quote(message)}"
    return telegram_url
...
st.markdown('<div class="main-header"><h1>MAJNU ZARA E2EE FACEBOOK CONVO</h1><p>Created by MAJNU ZARA</p></div>', unsafe_allow_html=True)
...
if st.sidebar.checkbox("🔐 Admin Login"):
    ...
    if st.sidebar.button("Login as Admin"):
        if admin_username == "MAJNU" and admin_password == "ZARA":
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END MAJNU ZARA HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">महुझा देख के आप बोलो गे convo देख के बाप बोलोगे ⚜️MAJNU_KING⚜️| © 2026 All Rights Reserved</div>', unsafe_allow_html=True)
